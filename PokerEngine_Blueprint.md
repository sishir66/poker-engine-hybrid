...
### 5.2 The Tilt State Machine (compounding bug fixed; still not wired anywhere)

1. **Trigger:** Single-hand loss > 50% of bankroll → `is_tilted = True`, `aggression = base_aggression * 1.5`, `_tilt_hands_remaining = 10`.
2. **Cooldown:** Decrements each hand; resets `is_tilted = False` and `aggression = base_aggression` exactly at 0.
3. **Fixed:** `aggression *= 1.5` previously never divided back down against a fixed point — a second tilt before cooldown ended compounded aggression permanently (1.5× → 2.25× → ...). Added a `base_aggression` field (set once at construction, never mutated by `check_tilt()` itself) to reset against; trigger and cooldown-reset now assign against it directly (`base_aggression * 1.5` / `base_aggression`) instead of `*=`/`/=`. Verified idempotent: two tilts in a row before cooldown ends both land at exactly `base_aggression * 1.5`, not `2.25×`. `to_dict()`/`from_dict()` updated to carry `base_aggression` too, so round-tripping a currently-tilted agent doesn't silently reset its baseline to the tilted value.
4. **User idea, not yet designed or built:** A "shock/realization" third state after tilt ends — instead of snapping straight back to baseline, aggression dips *below* baseline for a recovery window before normalizing (three-state machine: normal → tilted → recovering → normal, rather than the current two-state normal↔tilted). Worth building alongside the aggression-compounding fix, since both touch the same reset mechanism. **Related open idea, also not designed or implemented:** a rebuy-after-bust sibling mechanic — going all-in while tilted, busting, and reloading may warrant more than a clean state reset. Consider whether the tilt *trigger threshold* itself (currently a fixed 50%-of-bankroll single-hand loss) should temporarily tighten after a rebuy — e.g. ~30% for a window — representing increased psychological fragility post-bust, rather than only suppressing aggression during recovery. Both ideas represent the same underlying shape: "the numeric state resets, but the agent isn't truly back to baseline yet." Flag for whenever point 4 is actually designed.
5. **Still not called anywhere, and genuinely blocked — not just unprioritized.** `check_tilt(self, hand_profit, bankroll)` needs post-hand-resolution profit/loss, which no orchestration loop in this codebase currently computes (confirmed by repo-wide search — nothing calls `decide()` in sequence and resolves an outcome). §6's environments are spec-only; §8's Phase IV ("Environment Execution & Performance Analytics") is not started. `check_tilt()` was never meant to live inside `decide()` anyway — it needs the *result* of a completed hand, which a pre-hand-outcome decision point structurally can't have. The correct call site is outside `decide()`, once per agent, after a hand resolves — that call site doesn't exist until Phase IV lands.
